...
- `usr/bin/openwifi_ap`: AP mode helper (hostapd + udhcpd). Still degraded for deployment, but intentionally kept for controlled AP-side testing; see `docs/todo/todo_openwifi_tx_auth_failure.md`
...
- `root/openwifi/user_space/openwifi_perf_snapshot.sh`: one-shot STA/AP performance snapshot (link, station dump, TX/RX stats, queue state, key dmesg markers, rc_stats when available)
